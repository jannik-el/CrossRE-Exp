...
for DOMAIN in $domains; do
  ...
  for rs in "${!SEEDS[@]}"; do
    ...
    if [ -f "$EXP_PATH/${DOMAIN}/summary-exps.txt" ]; then
      echo "RS: ${SEEDS[$rs]}" >> $EXP_PATH/${DOMAIN}/summary-exps.txt
    else
      echo "Domain ${DOMAIN}" > $EXP_PATH/${DOMAIN}/summary-exps.txt
      echo "RS: ${SEEDS[$rs]}" >> $EXP_PATH/${DOMAIN}/summary-exps.txt
    fi
    ...
    python3 evaluate.py \
            ...
            --out_path ${exp_dir} \
            --summary_exps $EXP_PATH/${DOMAIN}/summary-exps.txt
