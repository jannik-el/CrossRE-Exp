...
def parse_arguments():
    ...
    arg_parser.add_argument(
        "-rs", "--seed", type=int, help="Seed for probabilistic components"
    )
...
def set_experiments(out_path, prediction=False):
    ...
    if prediction:
        logging.basicConfig(
            filename=os.path.join(out_path, "eval.log"),
            filemode="w",
            format=log_format,
            level=log_level,
        )
        ...
        logging.basicConfig(
            filename=os.path.join(out_path, "classify.log"),
            filemode="w",
            format=log_format,
            level=log_level,
        )
...
def run(
    classifier, criterion, optimizer, dataset, mode="train", return_predictions=False
):
    ...
    batch_idx = 0
    for domains, sentences, entities_1, entities_2, labels in dataset:
        batch_idx += 1
        ...
        if mode == "train":
            ...
            predictions = classifier(
                list(sentences), entities_1, entities_2, list(domains)
            )
        ...
        evaluation_metrics = criterion.get_classification_report(
        ...
        if return_predictions:
            ...
            for sidx in range(predictions["labels"].shape[0]):
                ...
                stats["predictions"].append(
                    predicted_labels[predicted_labels != -1].item()
                )
...
def save_predictions(path, data, pred_labels):
    ...
    with open(path, "w", encoding="utf8", newline="") as output_file:
        ...
        for _, sentences, entities1, entities2, _ in data:
...
def save_plots(path, loss_train, loss_dev, microf1, macrof1, weightedf1):
    ...
    plt.grid(linestyle="--")
    plt.plot(loss_train, color="darkturquoise", linestyle="-", label="loss train")
    plt.plot(loss_dev, color="limegreen", linestyle="-", label="loss dev")
    plt.xlabel("Epochs")
    ...
    plt.grid(linestyle="--")
    plt.plot(microf1, color="gold", linestyle="-", label="micro-f1")
    plt.plot(macrof1, color="red", linestyle="-", label="macro-f1")
    plt.plot(weightedf1, color="darkorange", linestyle="-", label="weighted-f1")
    plt.xlabel("Epochs")
    plt.legend()
    plt.tight_layout()
    plt.savefig(path_fig)
...
if __name__ == "__main__":
    ...
    classifier = classifier_constructor(
        emb_model=embedding_model, label_types=label_types.keys()
    )
    ...
    if args.prediction_only:
        ...
        if not os.path.exists(classifier_path):
            logging.error(
                f"[Error] No pre-trained model available in '{classifier_path}'. Exiting."
            )
            exit(1)
    ...
    if args.prediction_only:
        stats = run(
            classifier, criterion, None, test_data, mode="eval", return_predictions=True
        )
        ...
        pred_path = os.path.join(
            args.exp_path,
            f"{os.path.splitext(os.path.basename(args.test_path))[0]}-pred.csv",
        )
    ...
    for ep_idx in range(args.epochs):
        ...
        ep_stats = run(classifier, criterion, optimizer, train_data, mode="train")
        ...
        ep_stats = run(classifier, criterion, None, dev_data, mode="eval")
        ...
        if (ep_idx - stats["loss"].index(min(stats["loss"]))) >= args.early_stop:
            logging.info(
                f"No improvement since {args.early_stop} epochs ({min(stats['loss']):.4f} loss). Early stop."
            )
            break
