...
class DatasetMapper(Dataset):
    def __init__(self, domains, sentences, entities_1, entities_2, relations):
...
def prepare_data(data_path, labels2id, batch_size):
    ...
    domains, sentences, entities_1, entities_2, relations = read_json_file(
        data_path, labels2id
    )
    data_loader = DataLoader(
        DatasetMapper(domains, sentences, entities_1, entities_2, relations),
...
def read_json_file(json_file, labels2id, multi_label=False):
    ...
    domains, sentences, entities_1, entities_2, relations = [], [], [], [], []

    with open(json_file) as data_file:
        for json_elem in data_file:
            document = json.loads(json_elem)

            domain = document["doc_key"].split("-")[0].upper()
    ...
    return domains, sentences, entities_1, entities_2, relations
